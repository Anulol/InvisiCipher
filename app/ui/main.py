...
from app.models.DEEP_STEGO.video_stego import hide_video, reveal_video
...
class MainAppWindow(QMainWindow):
    def __init__(self):
        ...
        self.secret_video_filepath = None
        self.cover_video_filepath = None
        self.stego_video_filepath = None
        self.video_hide_status_label = None
        self.video_reveal_status_label = None
        self.secret_video_path_label = None
        self.cover_video_path_label = None
        self.stego_video_path_label = None
        ...
        video_hiding_button = QPushButton("Video Hide")
        image_reveal_button = QPushButton("Image Reveal")
        video_reveal_button = QPushButton("Video Reveal")
        ...
        video_hiding_button.clicked.connect(self.show_video_hiding_page)
        image_reveal_button.clicked.connect(self.show_reveal_page)
        video_reveal_button.clicked.connect(self.show_video_reveal_page)
    ...
    def show_video_hiding_page(self):
        self.main_content.set_background_image("assets/components_backgrounds/main_window_bg.png")
        self.secret_video_filepath = None
        self.cover_video_filepath = None
        self.clear_main_layout()

        title_label = QLabel("<H2>Video Steganography : Hide</H2>")
        title_label.setStyleSheet("font-size: 24px; color: #ffffff;")
        title_label.setAlignment(Qt.AlignTop)
        self.main_layout.addWidget(title_label)

        hint_label = QLabel("Upload a secret video and choose a cover video/image.")
        hint_label.setStyleSheet("font-size: 15px; color: #c6c6c6;")
        hint_label.setAlignment(Qt.AlignTop)
        self.main_layout.addWidget(hint_label)

        self.secret_video_path_label = QLabel("Secret video: Not selected")
        self.secret_video_path_label.setStyleSheet("font-size: 14px; color: #ffffff;")
        self.main_layout.addWidget(self.secret_video_path_label)

        self.cover_video_path_label = QLabel("Cover (video/image): Not selected")
        self.cover_video_path_label.setStyleSheet("font-size: 14px; color: #ffffff;")
        self.main_layout.addWidget(self.cover_video_path_label)

        self.video_hide_status_label = QLabel("Output: Not generated")
        self.video_hide_status_label.setStyleSheet("font-size: 14px; color: #c6c6c6;")
        self.main_layout.addWidget(self.video_hide_status_label)

        button_layout = QHBoxLayout()
        clear_button = QPushButton("Clear")
        clear_button.clicked.connect(self.show_video_hiding_page)
        button_layout.addWidget(clear_button)

        browse_secret_video_button = QPushButton("Browse secret video")
        browse_secret_video_button.clicked.connect(self.select_secret_video)
        button_layout.addWidget(browse_secret_video_button)

        browse_cover_button = QPushButton("Browse cover video/image")
        browse_cover_button.clicked.connect(self.select_cover_video_or_image)
        button_layout.addWidget(browse_cover_button)

        hide_video_button = QPushButton("Hide Video")
        hide_video_button.clicked.connect(self.perform_video_hide)
        button_layout.addWidget(hide_video_button)

        download_button = QPushButton("Download🔽")
        download_button.setEnabled(False)
        download_button.clicked.connect(self.download_image)
        self.download_steg_button = download_button
        button_layout.addWidget(download_button)
    ...
    def show_video_reveal_page(self):
        self.main_content.set_background_image("assets/components_backgrounds/main_window_bg.png")
        self.stego_video_filepath = None
        self.clear_main_layout()

        title_label = QLabel("<H2>Video Steganography : Reveal</H2>")
        title_label.setStyleSheet("font-size: 24px; color: #ffffff;")
        title_label.setAlignment(Qt.AlignTop)
        self.main_layout.addWidget(title_label)

        hint_label = QLabel("Upload a generated stego video and reveal the hidden video.")
        hint_label.setStyleSheet("font-size: 15px; color: #c6c6c6;")
        hint_label.setAlignment(Qt.AlignTop)
        self.main_layout.addWidget(hint_label)

        self.stego_video_path_label = QLabel("Stego video: Not selected")
        self.stego_video_path_label.setStyleSheet("font-size: 14px; color: #ffffff;")
        self.main_layout.addWidget(self.stego_video_path_label)

        self.video_reveal_status_label = QLabel("Output: Not generated")
        self.video_reveal_status_label.setStyleSheet("font-size: 14px; color: #c6c6c6;")
        self.main_layout.addWidget(self.video_reveal_status_label)

        button_layout = QHBoxLayout()
        clear_button = QPushButton("Clear")
        clear_button.clicked.connect(self.show_video_reveal_page)
        button_layout.addWidget(clear_button)

        browse_stego_button = QPushButton("Browse stego video")
        browse_stego_button.clicked.connect(self.select_stego_video)
        button_layout.addWidget(browse_stego_button)

        reveal_video_button = QPushButton("Reveal Video")
        reveal_video_button.clicked.connect(self.perform_video_reveal)
        button_layout.addWidget(reveal_video_button)

        download_button = QPushButton("Download🔽")
        download_button.setEnabled(False)
        download_button.clicked.connect(self.download_image)
        self.download_revealed_secret_image_button = download_button
        button_layout.addWidget(download_button)
    ...
    def generateImage(self, label):
        print("Image gen")
        gen_image_path = self.app_root / "generated_image.png"
        ...
        try:
            ...
            image.save(str(gen_image_path))
            pixmap = QPixmap(str(gen_image_path))
            label.setPixmap(pixmap.scaled(400, 400, Qt.KeepAspectRatio))
            self.latest_output_path = gen_image_path
            self.download_genimage_button.setEnabled(True)
        except Exception as e:
            QMessageBox.critical(self, "Generating error", f"Failed to generate the image.\n{e}")

    def show_random_text(self, textbox):
        import json, random
        lucky_json_path = self.app_root / "ui" / "assets" / "json" / "lucky.json"
        with open(lucky_json_path, "r") as f:
    ...
    def upscaleImage(self, label):
        ...
        model_path = self.app_root / "models" / "ESRGAN" / "models" / "RRDB_ESRGAN_x4.pth"
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        model = arch.RRDBNet(3, 3, 64, 23, gc=32)
        model.load_state_dict(torch.load(str(model_path), map_location=device), strict=True)
        model.eval()
        ...
        if os.path.exists(high_res_image_path):
            ...
            self.latest_output_path = Path(high_res_image_path)
    ...
    def download_image(self):
        if self.latest_output_path is None or not Path(self.latest_output_path).exists():
            QMessageBox.information(self, "Download", "No output file available to download yet.")
            return

        source = Path(self.latest_output_path)
        destination, _ = QFileDialog.getSaveFileName(self, "Save Output As", source.name)
        if not destination:
            return

        destination_path = Path(destination)
        if destination_path.suffix == "":
            destination_path = destination_path.with_suffix(source.suffix)

        try:
            shutil.copy2(source, destination_path)
            QMessageBox.information(self, "Download", f"Saved to:\n{destination_path}")
        except Exception:
            QMessageBox.critical(self, "Download Error", "Failed to save the output file.")
    ...
    def select_secret_video(self):
        ...
        if filepath:
            self.secret_video_filepath = filepath
            self.secret_video_path_label.setText(f"Secret video: {filepath}")

    def select_cover_video_or_image(self):
        file_dialog = QFileDialog()
        filepath, _ = file_dialog.getOpenFileName(
            self,
            "Select Cover Video or Image",
            "",
            "Media Files (*.mp4 *.avi *.mov *.mkv *.png *.jpg *.jpeg *.bmp);;All Files (*)",
        )
        if filepath:
            self.cover_video_filepath = filepath
            self.cover_video_path_label.setText(f"Cover (video/image): {filepath}")

    def select_stego_video(self):
        file_dialog = QFileDialog()
        filepath, _ = file_dialog.getOpenFileName(
            self,
            "Select Stego Video",
        ...
        if filepath:
            self.stego_video_filepath = filepath
            self.stego_video_path_label.setText(f"Stego video: {filepath}")
    ...
    def perform_hide(self, cover_filepath, secret_filepath):
        ...
        try:
            ...
            steg_image_path = self.app_root / "steg_image.png"
            pixmap = QPixmap(str(steg_image_path))
            self.steg_display_label.setPixmap(pixmap.scaled(256, 256, Qt.KeepAspectRatio))
            self.latest_output_path = steg_image_path
            self.download_steg_button.setEnabled(True)
        except:
            QMessageBox.critical(self, "Hiding Error", "Failed to hide the image.")

    def perform_video_hide(self):
        if self.secret_video_filepath is None or self.cover_video_filepath is None:
            QMessageBox.information(self, "Video Hiding Error", "Please select secret and cover files first.")
            return
        try:
            output_video = self.app_root / "stego_video.avi"
            output_frames = self.app_root / "stego_video_frames"
            output_video, output_frames, frame_count = hide_video(
                self.secret_video_filepath,
                self.cover_video_filepath,
                output_video,
                output_frames,
            )
            self.latest_output_path = output_video
            self.download_steg_button.setEnabled(True)
            self.video_hide_status_label.setText(
                f"Output: {output_video} | Frames: {output_frames} | Count: {frame_count}"
            )
            self.video_hide_status_label.setStyleSheet("font-size: 14px; color: #00ff00;")
        except Exception as e:
            QMessageBox.critical(self, "Video Hiding Error", f"Failed to hide the video.\n{e}")

    def perform_video_reveal(self):
        if self.stego_video_filepath is None:
            QMessageBox.information(self, "Video Reveal Error", "Please select the steg video first.")
            return
        try:
            output_video = self.app_root / "revealed_secret_video.avi"
            output_frames = self.app_root / "revealed_secret_frames"
            output_video, output_frames, frame_count = reveal_video(
                self.stego_video_filepath,
                output_video,
                output_frames,
            )
            self.latest_output_path = output_video
            self.download_revealed_secret_image_button.setEnabled(True)
            self.video_reveal_status_label.setText(
                f"Output: {output_video} | Frames: {output_frames} | Count: {frame_count}"
            )
            self.video_reveal_status_label.setStyleSheet("font-size: 14px; color: #00ff00;")
        except Exception as e:
            QMessageBox.critical(self, "Video Reveal Error", f"Failed to reveal video.\n{e}")
    ...
    def perform_reveal(self, filepath):
        ...
        try:
            ...
            secret_out_filepath = self.app_root / "secret_out.png"
            pixmap = QPixmap(str(secret_out_filepath))
            self.secret_out_display_label.setPixmap(pixmap.scaled(256, 256, Qt.KeepAspectRatio))
            self.latest_output_path = secret_out_filepath
            self.download_revealed_secret_image_button.setEnabled(True)
        except:
    ...
    def perform_encryption(self, filepath):
        ...
        try:
            ...
            self.latest_output_path = Path(filepath + ".enc")
        ...
        try:
            if self.aes_radio_dec.isChecked():
                ...
                if result == -1:
                    QMessageBox.critical(self, "Decrypting error", "Wrong Key!                    ")
                    return
            self.latest_output_path = Path(dec_filename)
    ...
    def load_stylesheet(self):
        stylesheet_path = self.app_root / "ui" / "styles" / "style.qss"
        stylesheet = QFile(str(stylesheet_path))
